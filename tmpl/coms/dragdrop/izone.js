...
var Bar = {
    create: function() {
        var node = $('#' + barId);
        if (!node.length) {
            $(document.body).append('<div id="' + barId + '" class="@izone.css:bar"><div/></div>');
        }
    },
    hide: function() {
        $('#' + barId).hide();
    ...
    update: function(anchor, bound, side) {
        ...
        if (anchor & IDragdropZone.ANCHOR_HORIZONTAL) {
            bar.removeClass('@izone.css:vbar').addClass('@izone.css:hbar');
            bar.css({
                width: bound.width - 6,
                height: 2,
                display: 'block'
        ...
        if (anchor & IDragdropZone.ANCHOR_VERTICAL) {
            bar.removeClass('@izone.css:hbar').addClass('@izone.css:vbar');
            bar.css({
...
var Pointer = {
    create: function() {
        var node = $('#' + pointerId);
        if (!node.length) {
            $(document.body).append('<div id="' + pointerId + '" class="@izone.css:pointer"/>');
        }
    },
    hide: function() {
        $('#' + pointerId).hide();
    ...
    update: function(f, cfg) {
        var green = '@izone.css:enable',
            red = '@izone.css:disable';
        var me = this;
        if (me.$f != f) {
            me.$f = f;
