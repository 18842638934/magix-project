define('coms/inputag/index',['magix','$','../monitor/index','../updater/index','../tmpl/index'],function(require,exports,module){
/*Magix ,$ ,Monitor ,Updater ,tmpl */
/*
...
var html = "<div class=\"mp-752-tags clearfix\" id=\"tags_<%=id%>\" mx-click=\"showList()\" mx-guid=\"x7141-\u001f\">@1-\u001f</div><div class=\"mp-752-list mp-752-none\" id=\"list_<%=id%>\"><ul mx-guid=\"x7142-\u001f\">@2-\u001f</ul></div>";
var htmlData = [{"guid":1,"keys":["selected"],"tmpl":"<%for(var i=0;i<selected.length;i++){%><div class=\"mp-752-item\" id=\"si_<%=i%>_<%=id%>\" title=\"<%=selected[i]%>\"><div class=\"mp-752-name ellipsis\"><%=selected[i]%></div><div class=\"mp-752-delete\" mx-click=\"remove({index:<%=i%>})\">x</div></div><%}%><div class=\"mp-752-item\" id=\"iptw_<%=id%>\"><input id=\"ipt_<%=id%>\" class=\"mp-752-ipt\" mx-keyup=\"search()\" mx-paste=\"search()\" value=\"<%=iptValue%>\"/></div>","selector":"div[mx-guid=\"x7141-\u001f\"]"},{"guid":2,"keys":["list"],"tmpl":"<%for(var i=0;i<list.length;i++){%><li id=\"idx_<%=i%>\" class=\"mp-752-li ellipsis\" mx-mouseout=\"hover()\" mx-mouseover=\"hover()\" mx-click=\"fill({text:'<%=list[i].text%>'})\" data-text=\"<%=list[i].text%>\"><%=list[i].text%></li><%}%>","selector":"ul[mx-guid=\"x7142-\u001f\"]"}];
