define('app/views/demos/sdk',['magix','./partials/sdk-bigimg'],function(require,exports,module){
/*Magix ,BigImg */
/*
...
module.exports = Magix.View.extend({
    tmpl: "<table class=\"table mx-d1a-w700 mx-d1a-m50\"><thead><tr><th colspan=\"3\">名称</th></tr></thead><tbody><%for(var i=0;i<items.length;i++){%><tr><td class=\"mx-d1a-vm mx-d1a-w100\"><label><input type=\"checkbox\" mx-click=\"store({id:'<%=items[i].id%>'})\" id=\"cb_<%=id%>_<%=items[i].id%>\"/><%=items[i].name%></label></td><td mx-mouseover=\"hoverBig({img:'<%=items[i].bigImg%>'})\" mx-mouseout=\"hoverBig()\"><img src=\"<%=items[i].img%>\"/></td><td class=\"mx-d1a-vm\"><%=items[i].description%></td></tr><%}%></tbody></table><div class=\"mx-d1a-float\" mx-guid=\"xff41-\u001f\">@1-\u001f</div>",
    tmplData: [{"guid":1,"keys":["selected"],"tmpl":"<ul><%for(var i=0;i<selected.length;i++){%><li class=\"mx-d1a-sitem\"><%=selected[i].name%><a href=\"#!/index\" class=\"mx-d1a-fr\" mx-click=\"remove({index:<%=i%>})\">删除</a></li><%}%></ul>","selector":"div[mx-guid=\"xff41-\u001f\"]"}],
    ctor: function() {
        var me = this;
