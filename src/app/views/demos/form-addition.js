...
module.exports = View.extend({
    tmpl: "<div class=\"mp-1cd-wrapper\"><ul mx-guid=\"xff61-\u001f\">@1-\u001f</ul></div><hr/><div class=\"mp-1cd-wrapper\"><ul mx-guid=\"xff62-\u001f\">@2-\u001f</ul></div><button mx-click=\"saveSnapshot()\" class=\"btn\">保存当前数据</button>",
    tmplData: [{"guid":1,"keys":["list"],"tmpl":"<li><input class=\"input\" type=\"text\" value=\"<%=list[0].test%>\" mx-change=\"setValue({path:'list.0.test'})\"/></li><%for(var i=1;i<list.length;i++){%><li><input class=\"input\" type=\"text\" mx-change=\"setValue({path:'list.<%=i%>.test'})\" value=\"<%=list[i].test%>\"/><button class=\"btn\" mx-click=\"remove({index:<%=i%>})\" style=\"margin-left:10px\">X</button></li><%}%><li><button mx-click=\"add()\" class=\"btn\">添加一个</button></li>","selector":"ul[mx-guid=\"xff61-\u001f\"]"},{"guid":2,"keys":["platforms"],"tmpl":"<%for(var i=0;i<platforms.length;i++){%><li><div mx-view=\"app/views/demos/partials/dropdown?type=platform&selected=<%=platforms[i].platformId%>\" mx-change=\"setValue({path:'platforms.<%=i%>.platformId'})\" class=\"mp-1cd-fl mp-1cd-ml10\"></div><div mx-view=\"app/views/demos/partials/dropdown?type=operator&selected=<%=platforms[i].operatorId%>\" mx-change=\"setValue({path:'platforms.<%=i%>.operatorId'})\" class=\"mp-1cd-fl mp-1cd-ml10\"></div><input class=\"input mp-1cd-w80 mp-1cd-ml10\" type=\"text\" mx-change=\"setValue({path:'platforms.<%=i%>.version'})\" value=\"<%=platforms[i].version%>\" placeholder=\"版本号\"/> <button class=\"btn mp-1cd-ml10\" mx-click=\"removePlatform({index:<%=i%>})\">X</button></li><%}%><li><button mx-click=\"addPlatform()\" class=\"btn\">添加一个平台</button></li>","selector":"ul[mx-guid=\"xff62-\u001f\"]"}],
    ctor: function() {
        var me = this;
