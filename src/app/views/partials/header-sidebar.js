...
module.exports = Magix.View.extend({
    tmpl: "<ul class=\"mp-286-menus\" mx-guid=\"xe4b1-\u001f\">@1-\u001f</ul>",
    tmplData:[{"guid":1,"keys":["menus","url"],"tmpl":"<%for(var i=0,menu;i<menus.length;i++){menu=menus[i]%><li><a href=\"#!<%=menu.url%>\" <%if(menu.url==url){%> class=\"mp-286-on\" <%}%>><%if(menu.icon){%><i class=\"iconfont\"><%=menu.icon%></i><%}%> <%=menu.text%></a></li><%}%>","selector":"ul[mx-guid=\"xe4b1-\u001f\"]"}],
    ctor: function() {
        var me = this;
        me.$updater.set({
            menus: []
